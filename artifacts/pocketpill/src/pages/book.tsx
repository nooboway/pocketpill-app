...
import { loadSettings, getWhatsappUrl, getPaypalLink, type AdminSettings } from "@/lib/adminSettings";
              ...
              <div className="flex items-center justify-between mb-3">
                {STEPS.slice(0, -1).map((label, i) => (
                  <div key={i} className={`flex items-center gap-2 text-[11px] tracking-[0.15em] uppercase font-medium transition-colors duration-300 ${i === step ? "text-primary" : i < step ? "text-primary/60" : "text-muted-foreground/40"}`}>
                    <span className={`w-5 h-5 rounded-full border flex items-center justify-center text-[10px] transition-all duration-300 ${i < step ? "bg-primary border-primary text-primary-foreground" : i === step ? "border-primary text-primary" : "border-border/40 text-muted-foreground/40"}`}>
  ...
  return (
    <div>
      ...
      <div className="mt-8 flex items-center gap-3 text-xs text-muted-foreground/60">
        <Shield className="w-4 h-4 text-primary/50" strokeWidth={1.5} />
...
function StepDetails({
  ...
  tier: ReturnType<typeof loadSettings>["tiers"][0] | null;
  details: Details;
  ...
  return (
    <div>
      ...
      <div className="space-y-5">
        ...
        <Field label="WhatsApp number" icon={<Phone className="w-4 h-4" />} error={errors.whatsapp} hint="Include country code, e.g. +234 801 234 5678">
          <input
            type="tel"
        ...
        <Field label="Email address" icon={<Mail className="w-4 h-4" />} error={errors.email} hint="For your booking confirmation">
          <input
            type="email"
        ...
        <Field label="Brief description" icon={<MessageCircle className="w-4 h-4" />} error={errors.concern} hint="A sentence or two — this helps your pharmacist prepare">
          <textarea
      ...
      <div className="mt-8 flex items-center gap-3 text-[11px] text-muted-foreground/60">
        <Lock className="w-3.5 h-3.5 text-primary/50" strokeWidth={1.5} />
    ...
    <div>
      ...
      {!paymentConfirmed ? (
        <div className="space-y-4">
          ...
          >
            <svg width="20" height="24" viewBox="0 0 124 33" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-5 w-auto">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="bg-primary/[0.07] border border-primary/25 rounded-sm p-6 flex items-center gap-4">
          ...
          {dates.map((d, i) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center py-3 px-2 rounded-sm border text-center transition-all duration-200 ${
                  isSelected
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
      <div className="text-center mb-12">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200, damping: 15, delay: 0.2 }} className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-primary/10 border border-primary/30 mb-6">
      ...
      <div className="bg-card border border-border/50 rounded-sm p-7 mb-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={onCopy} className="flex items-center gap-2 font-mono text-primary font-semibold text-sm hover:text-primary/80 transition-colors">
...
function Field({
  ...
  return (
    <div>
      ...
      {error && <p className="text-[11px] text-destructive/80 mt-1.5">{error}</p>}
    </div>
  );
}

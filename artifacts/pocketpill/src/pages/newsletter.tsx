...
export default function NewsletterPage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-background flex flex-col selection:bg-primary/30">
      {/* Nav */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-b border-border/40 py-4">
          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-start">
            ...
            <motion.div initial="hidden" animate="visible" variants={STAGGER}>
              ...
              <motion.div variants={STAGGER} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {TOPICS.map((topic, i) => (
                  <motion.div key={i} variants={FADE_UP} className="flex items-start gap-4 p-4 border border-border/30 rounded-sm hover:border-primary/20 transition-colors bg-card/20">
                    <span className="text-primary/70 mt-0.5 shrink-0">{topic.icon}</span>
                    <div>
                  ...
                  {!submitted ? (
                    <>
                      ...
                      <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                          <label className="block text-xs uppercase tracking-wider font-medium text-foreground/70 mb-2">First name <span className="text-muted-foreground/50 normal-case tracking-normal">(optional)</span></label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-xs uppercase tracking-wider font-medium text-foreground/70 mb-2">Email address</label>
                          <input
                            type="email"
                            ...
                            required
                          />
                          {error && <p className="text-xs text-destructive/80 mt-1.5">{error}</p>}
                        </div>
                      ...
                      <div className="mt-6 flex items-center gap-2 text-xs text-muted-foreground/60">
                        <Shield className="w-3.5 h-3.5 text-primary/50 shrink-0" strokeWidth={1.5} />
                        <span>No spam. No sharing your data. Unsubscribe with one click, any time.</span>
                      </div>
                    </>
                  ) : (
                    <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className="text-center py-6">
                      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 200, damping: 15 }} className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-primary/10 border border-primary/30 mb-6">
